...
SOURCES = bq~.c tabosc4c~.c tabosci~.c tabread4c~.c tabread2d~.c

# list all pd objects (i.e. myobject.pd) files here, and their helpfiles will
# be included automatically
PDOBJECTS = ADSR.pd bq_coef_notch.pd oscillo~.pd bq_coef_peak.pd bq_coef_bp.pd pwm~.pd bq_coef_highshelf.pd compress_limit~.pd compress_limit_sidechain~.pd saw~.pd bq_coef_hip.pd distortion~.pd spatialisation~.pd bq_coef_lop.pd echo~.pd bq_coef_lowshelf.pd granulator~.pd distortion2~.pd saw2~.pd bq_list~.pd triangle~.pd
...
EXTRA_DIST = _sinh.pd isinc.wav sinc.wav triangle_table_generation.pd saw2_table_generation.pd
